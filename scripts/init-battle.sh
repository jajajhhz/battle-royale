#!/usr/bin/env bash
# init-battle.sh — Scaffold a new battle directory with stub files.
#
# Usage:
#   init-battle.sh <target-dir> [--name "Battle Name"] [--rubric balanced] [--contestants N]
#
# Default N = 4. Any N >= 2 is supported. The bracket is built dynamically
# by advance.sh from the contestant list; no need to specify pairings.

set -euo pipefail

target="${1:?Usage: init-battle.sh <target-dir> [--name NAME] [--rubric NAME] [--contestants N]}"
shift || true
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --contestants) contestants="$2"; shift 2 ;;
    *) echo "Unknown arg: $1" >&2; exit 1 ;;
  esac
done

if ! [[ "$contestants" =~ ^[0-9]+$ ]] || [[ "$contestants" -lt 2 ]]; then
  echo "ERROR: --contestants must be an integer >= 2 (got: $contestants)" >&2
  exit 1
fi
if [[ "$contestants" -gt 32 ]]; then
  echo "ERROR: refusing to scaffold $contestants contestants (max 32). Cull first." >&2
  exit 1
fi
...
# Generate the contestants list dynamically.
contestants_block=""
for ((i=1; i<=contestants; i++)); do
  contestants_block+="  - id: \"$i\"
    name: \"Idea $i\"
    spec: \"ideas/idea-$i.md\"
"
done
...
# input order (single-elimination with byes for non-powers-of-2).
# Override here only if you want explicit seeding. Example:
#   bracket:
#     round-1:
#       - ["1", "3"]
#       - ["2", "4"]
...
echo "Battle scaffolded at: $target  ($contestants contestants)"
echo
echo "Next steps:"
echo "  1. Edit $target/battle.yaml — fill in real contestant names/specs"
echo "  2. Write $target/ideas/idea-{1..$contestants}.md with full specs"
...
echo "  quick-battle.sh <idea1.md> <idea2.md> [<idea3.md> ...]"
